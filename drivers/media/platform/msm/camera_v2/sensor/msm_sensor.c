...
#include <linux/productinfo.h> //hisense add for register productinfo
#undef CDBG
				  ...
				  const void *data)
{
	...
	cci_client->id_map = 0;
	cci_client->i2c_freq_mode = I2C_FAST_MODE;//force I2C freq fast mode for yuv sensor  by hisense
	if (!s_ctrl->func_tbl)
