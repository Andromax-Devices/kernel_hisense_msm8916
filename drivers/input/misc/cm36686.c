...
static struct workqueue_struct *cm36686_workqueue = NULL;
...
static int cm36686_cross_talk_calibration(struct i2c_client *client)
{
	...
	if (ret < 0) {
		printk("write PS_CONF12 err in function cross_talk_calibration\n");
		return ret;
	}

	for (i = 0; i < 4; i++){	
		msleep(18);//change according to I AND T
		test_pdata =  i2c_smbus_read_word_data( client,REG_PS_DATA );
		
		 if(test_pdata<=PS_CROSSTALK_MAX){
			if((test_pdata/100)>0){
			//need to check if the canc now is not 0  and  should add it       what should do  if  crasstalk change small
				lpi->ps_crosstalk.hundred=(test_pdata/100);                 //if ps_data is 0 we should del the canc 
			 	ret=i2c_smbus_write_word_data(client,REG_PS_CANC , ((lpi->ps_crosstalk.hundred)*100));
				if (ret < 0) {
				printk("%s(): write PS_CANC error 0000!\n", __func__);	
				}
			}
			else if((test_pdata/50)>0){
				lpi->ps_crosstalk.fifty=(test_pdata/50);
				ret=i2c_smbus_write_word_data(client,REG_PS_CANC ,
						((lpi->ps_crosstalk.hundred)*100+ (lpi->ps_crosstalk.fifty)*50));
				if (ret < 0) {
				printk("%s(): write PS_CANC error 1111!\n", __func__);	
				}
			}
			else if((test_pdata/10)>0){
				lpi->ps_crosstalk.ten=(test_pdata/10);
				lpi->last_ps_canc= ((lpi->ps_crosstalk.hundred)*100+ (lpi->ps_crosstalk.fifty)*50
							+(lpi->ps_crosstalk.ten)*10);
				ret=i2c_smbus_write_word_data(client,REG_PS_CANC ,((lpi->ps_crosstalk.hundred)*100+ 
						(lpi->ps_crosstalk.fifty)*50+(lpi->ps_crosstalk.ten)*10));
				if (ret < 0) {
				    printk("%s(): write PS_CANC error 2222!\n", __func__);	
				}
	...
	// if the crosstalk is too large we do not calibrate but use the last_ps_canc
	ret=i2c_smbus_write_word_data(client,REG_PS_CANC ,lpi->last_ps_canc);
	if (ret < 0) {
	    printk("%s(): write PS_CANC error 4444!\n", __func__);	
	}
	...
	if (ret < 0) {
        ...
        // printk(KERN_ERR "calibration not finish and ues last  calibration data\n");
	return 0;
}
...
static int psensor_function_on(struct CM36686_info *lpi)
{
	...
	/*2. set interrupt threshold value*/
	ret = i2c_smbus_write_word_data(lpi->i2c_client,REG_PS_THDL ,lpi->ps_away_thd_set);
	if (ret < 0) {
		dev_err(&lpi->i2c_client->dev,
			"%s(): write PS_THDL error!\n", __func__);
		return ret;
	...
	ret = i2c_smbus_write_word_data(lpi->i2c_client,REG_PS_THDH ,lpi->ps_close_thd_set );	
	if (ret < 0) {
		dev_err(&lpi->i2c_client->dev,
			"%s(): write PS_THDH error!\n", __func__);
		return ret;
...
static void ps_dwork_handler(struct work_struct *work)
{
	...
	printk("int_flag = 0x%x\n", int_flag);

	if (int_flag & INT_FLAG_PS_SPFLAG) {
		printk("Saturation caused the chip turn to protection mode,"
			" light on the LCD.");
		goto turn_on_lcd;
	}
	
	if ((int_flag & INT_FLAG_PS_IF_CLOSE) && (int_flag & INT_FLAG_PS_IF_AWAY)) {
		// error!
		printk("PS_NEAR_TO_FAR & PS_FAR_TO_NEAR occurred at same time,"
	...
	if (int_flag & INT_FLAG_PS_IF_CLOSE) {
		...
		printk(KERN_ALERT "far_to_near!\n");
		goto quit;
	}

	if (int_flag & INT_FLAG_PS_IF_AWAY) {
		printk(KERN_ALERT "near_to_far!\n");
		goto turn_on_lcd;
	}
	
	printk("error! Neither PS_FAR_TO_NEAR nor PS_NEAR_TO_FAR int occurred,"
		" but irq triggered.\n");
...
static irqreturn_t cm36686_ps_irq_handler(int irq, void *data)
{
	...
	if(!lpi->ps_function_on){
		printk("%s(): Chip not functioned, report nothing!\n",__func__);
		goto quit;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 0x0c; i++) {
		...
		ret += sprintf(buf + ret, "REG [%d]: 0x%x\n", i, temp_val);
	}
	return ret;
}
				...
				const char *buf, size_t count)
{
	...
	if ((code > 0) && (code < 65535)) {
		...
		if (!lpi->ps_function_on) {
			printk("PS function is DISABLED, do not set thds, but stored it!\n");
			return -EINVAL;
		}
		if (0 > i2c_smbus_write_word_data(lpi->i2c_client, REG_PS_THDH,lpi->ps_close_thd_set ))
		{
			dev_err(&lpi->i2c_client->dev,
		...
		printk("%s(): It is an illegal para, %d!", __func__, code);
		return -EINVAL;
	}
				...
				const char *buf, size_t count)
{
    ...
    if ((code > 0) && (code < 65535)) {
		...
		if(!lpi->ps_function_on){
			printk("ps function is off can not set the thd\n");
			return -EINVAL;
		}
		if (0 > i2c_smbus_write_word_data(lpi->i2c_client, REG_PS_THDL,lpi->ps_away_thd_set))
			 {
			dev_err(&lpi->i2c_client->dev,
				"%s(): write PS_THD error!\n", __func__);
			return -EIO;
                                ...
                                char *resolution_result)
{
    ...
    strcpy(resolution_result,resolution_lut[lpi->als_it_index]);
    printk("%s:resolution_result=%s\n",__func__,resolution_result);
    return 0;
}
#endif
...
{
	struct CM36686_info *lpi =
		container_of((struct delayed_work*)work, struct CM36686_info, als_dwork);
	u32 adc_value = 0;
	u32 temp_val = 0;

	if (0 == lpi->als_enable) {
		printk("%s(): als not enabled, do not re-schedule als_dwork!\n", __func__);
		return;
	}
	...
	adc_value =
		(temp_val * (1 << (3 - lpi->als_range)) * lpi->als_fittness)/ 100;
		...
		unsigned int enable)
{
	...
	printk("receive als_en is %d\n", enable);
	if (enable == lpi->als_enable)
		return 0;

	if (enable) {
		cancel_delayed_work(&lpi->als_dwork);
		//flush_delayed_work(&lpi->als_dwork);
		queue_delayed_work(cm36686_workqueue, &lpi->als_dwork,
			msecs_to_jiffies(CM36686_LS_DEFAULT_POLL_DELAY));
	} else {
		cancel_delayed_work(&lpi->als_dwork);
...
static int ps_irq_init(struct CM36686_info *lpi)
{
	...
	ret = request_irq(lpi->irq,
                                ...
                                lpi);
	if (ret) {
		printk(KERN_WARNING "Request IRQ for cm36686 failed, return:%d\n", ret);
		goto fail_free_intr_pin;
	}

	ret = enable_irq_wake(lpi->irq);
	if(0 != ret){
		printk(KERN_ERR " enable_irq_wake failed for cm36686_ps_irq_handler\n");
						...
						const struct i2c_device_id *id)
{
	...
	lpi->ps_cdev.sensors_poll_delay = NULL,
	...
	ret = sysfs_create_group(&((lpi->ps_cdev.dev)->kobj), &attribute_group_ps);
	if (ret)
		goto err_create_ps_device_file;
	
	ret = sysfs_create_group(&((lpi->als_cdev.dev)->kobj), &attribute_group_als);
	if (ret)
